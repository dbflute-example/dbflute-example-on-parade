...
public interface OnparadeEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/onparade */
    ...
    /** The key of the configuration. e.g. onparade-support@annie.example.com */
    ...
    /** The key of the configuration. e.g. localhost:8090/onparade */
     ...
     * The value is, e.g. /tmp/lastaflute/onparade <br>
     ...
     * The value is, e.g. onparade-support@annie.example.com <br>
     * comment: Mail Address for Onparade Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * The value is, e.g. localhost:8090/onparade <br>
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements OnparadeEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(OnparadeEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(OnparadeEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(OnparadeEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(OnparadeEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(OnparadeEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(OnparadeEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(OnparadeEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(OnparadeEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(OnparadeEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(OnparadeEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(OnparadeEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(OnparadeEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(OnparadeEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(OnparadeEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(OnparadeEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(OnparadeEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(OnparadeEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(OnparadeEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(OnparadeEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(OnparadeEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(OnparadeEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(OnparadeEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(OnparadeEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(OnparadeEnv.SERVER_DOMAIN);
        }
    }
}
