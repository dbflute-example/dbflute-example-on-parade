...
public class StartupLogic {
    ...
    public void toOnparade(File projectDir, String domain, String serviceName) {
        String packageName = buildPackageName(domain);
        new NewProjectCreator("onparade", projectDir, original -> {
            String filtered = original;
            filtered = replace(filtered, "lastaflute-example-onparade", Srl.initUncap(serviceName));
            ...
            filtered = replace(filtered, "Onparade", Srl.initCap(serviceName));
            filtered = replace(filtered, "onparade", Srl.initUncap(serviceName));
